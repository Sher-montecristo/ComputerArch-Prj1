...
intsig M_branchD 'ex_mem_curr->branchD' # branch destination for branch 
...
intsig W_ifun 'mem_wb_curr->ifun'	# Instruction code
...
int f_pc = [
	#TODO: 要按照prediction分类
	# misprediction not-taken ---> should take the branch des (in M_valA)
	M_icode == IJXX && M_Cnd && W_valE > 0 && W_ifun == ALUAND &&  : M_valA;            
...
int f_stat = [
	imem_error: SADR;            # Invalid memory address
	!instr_valid : SINS;         # Invalid instruction
	f_icode == IHALT : SHLT;     # Halt instruction encountered 
	1 : SAOK;                    # Normal execution
];
...
int f_predPC = [
	...
	f_icode == IJXX && d_valA <= 0 : f_valC; 	#branch taken prediction when jle | jmp
	f_ifun == UNCOND : f_valC;
	1 : f_valP;  								#branch not taken prediction
];
...
int d_valA = [
	#TODO-FINISHED: 当为branch not taken 预测时改为 D_valC
	D_icode == ICALL : D_valP;     # Use incremented PC
	D_icode == IJXX && E_valA <= 0 : D_valP;                #branch taken prediction
	D_icode == IJXX && E_valA > 0  : D_valC;			    #branch not-taken prediction
...
bool D_bubble =
	# Mispredicted branch
	# 是跳转指令，同时condition 不满足， 这个情况是针对branch alway taken的
	!(E_ifun == UNCOND) &&
	( 
	(E_icode == IJXX && !e_Cnd && ) ||
	(E_icode == IJXX && e_Cnd && M_valA > 0) ||
	(E_icode == IJXX && !e_Cnd && M_valA <= 0) ||
    #TODO-FINISHED: 当branch not taken预测错误时 要bubble
	# Stalling at fetch while ret passes through pipeline
