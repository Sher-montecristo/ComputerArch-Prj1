...
intsig CLE    'C_LE'			# Jump if LE
intsig CL     'C_L'			# Jump if L
intsig CE     'C_E'			# Jump if E
intsig CNE    'C_NE'			# Jump if NE
intsig CGE    'C_GE'			# Jump if GE
intsig CG     'C_G'			# Jump if G
...
boolsig f_cnd_alu 'cond_holds(compute_cc(id_ex_curr->ifun, gen_aluA(), gen_aluB()), if_id_next->ifun)'
...
int f_predPC = [
	...
	f_icode == IJXX && (E_icode in {IOPL, IIADDL}) && !f_cnd_alu : f_valP;

	 # Execute stage is not ALU -> check cc -> ZF SF OF
	f_icode == IJXX && !(E_icode in {IOPL, IIADDL}) && !f_cnd_other : f_valP;
